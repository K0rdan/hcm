...
const addHorseMutation = gql`
  mutation AddHorse($name: String!) {
    addHorse(name: $name) @client {
      horse {
        id
        name
      }
...
`;
export const withAddHorseMutation = graphql(addHorseMutation, {
  props: ({ mutate }) => ({
    addHorse: name => mutate({ variables: { name } }),
  }),
});
